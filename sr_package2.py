...
class package:
    ...
    def __init__(self, logname="", notes_name=sr.NOTES2):
        """__init__() -> package
        ...
        self.chain = ""
        self.prepost = ""
        self.prepost_filename = sr.PREPOSTLIB2
        self.next_p = None
    ...
    def _read_notes(self):
        ...
        fool = ""
        fool2 = ""
        f = open(sr.RUCKUS + "/package/" + self.notes_name, 'r')
        ...
        for i in self.srp_flags[:]:
            ...
            if x not in sr.supported_flags:
                ...
                if x == "SRP_INPLACE":
                    ...
                    self.chain = y
                elif x == "SRP_PREPOSTLIB":
                    utils.vprint("initializing self.prepostlib_name")
                    self.prepost_filename = y
        ...
        if "SRP_PREPOSTLIB" in self.srp_flags:
            ...
            modname = self.prepost_filename.split(".py")[0]
...
#        print "***** size_total: %s" % (size_total)
    ...
    def _build(self):
        ...
        """
        if not os.path.isdir(os.path.join(sr.RUCKUS, "build", self.dirname)):
...
./srp_go """
        ...
        try:
            ...
            if "SRP_PREPOSTLIB" in self.srp_flags:
                ...
                if os.path.exists(prepost) or os.path.islink(prepost):
                    os.unlink(prepost)
                os.link(sr.RUCKUS + "/package/" + self.prepost_filename,
                        prepost)
                ...
                f.write(temp + "_-_" + self.prepost_filename + "\n")
                
                # add checksum line, if configured
                fname = self.logfile + "_-_" + self.prepost_filename
                if utils.any_of_in(["SRP_MD5SUM",
                                    "SRP_SHA1SUM",
                                    "SRP_CHECKSUM"],
                                   self.srp_flags):
            ...
            if current != []:
                ...
                if current[0] == os.path.basename(self.logfile):
                    ...
                    if not sr.PERSISTENT:
                        ...
                        # PREPOSTLIB might not be named PREPOSTLIB.py
                        oldname = current[0] + "_-_" + self.prepost_filename
                        if os.path.exists(oldname) and self.prepost_filename:
                            newname = oldname + "_--_srp-upgrade-leftovers"
                            os.rename(oldname, newname)
    ...
    def info(self):
        ...
        if "SRP_CHAIN" in self.srp_flags:
            print "chain: %s" % (self.chain)
        if "SRP_PREPOSTLIB" in self.srp_flags:
            print "prepost_filename: %s" % (self.prepost_filename)
        ...
        while self.next_p:
            print
            self.next_p.info()
            self.next_p = self.next_p.next_p
    ...
    def install(self, force=0, current=[], previous=[]):
        ...
        # do a little cleanup
        for x in ["tmp"]:
            for y in os.listdir(os.path.join(sr.RUCKUS, x)):
                y = os.path.join(sr.RUCKUS, x, y)
                utils.vprint("cleaning: %s" % y)
                utils.nuke_fs_node(y)
        
        retval = 1
        # checking 'force' keeps us from entering this loop when upgrading.
        # when upgrading, the SRP_CHAIN looping is handled in the upgrade()
        # function.
        while self.next_p and retval and not force:
            print
            print "installing chained package: %s" % self.next_p.name
            retval = self.next_p.install(force=force,
                                         current=current,
                                         previous=previous)
            self.next_p = self.next_p.next_p
    ...
    def uninstall(self, quiet=False):
        ...
        retval: 1 = success, 0 = failure, -1 = not installed
        """
        # make quiet mode noisy if sr.VERBOSE is set
        if sr.VERBOSE:
            quiet = False

        status = self.check(quiet=quiet)
        # quiet mode is always persistent
        ...
        if "SRP_PREPOSTLIB" in self.srp_flags:
            ...
            try:
                ...
                os.link(temp + "_-_" + self.prepost_filename,
                        sr.RUCKUS + "/package/" + self.prepost_filename)
            except:
                pass
    ...
    def upgrade(self):
        ...
        for i in previous:
            ...
            while filename != '':
                ...
                temp3 = log.split("_--_")[0] + "_-_" + self.prepost_filename
                utils.vprint("temp1: %s" % temp1)
                utils.vprint("temp2: %s" % temp2)
                utils.vprint("temp3: %s" % temp3)
            ...
            if old_files != []:
                ...
                # remove leftovers if SRP_LEFTOVERS isn't set
                if not "SRP_LEFTOVERS" in srp_flags:
                    sys.stdout.write("removing leftovers...".ljust(69))
                    p = package(i)
                    old_state = sr.PERSISTENT
                    sr.PERSISTENT = 1
                    status = p.uninstall(quiet=True)
                    sr.PERSISTENT = old_state
                    if not status:
                        print "WARNING: leftover removal failed!"
                    else:
                        print "[  done  ]"
        ...
        # do SRP_CHAIN looping
        retval = 1
        while self.next_p and retval:
            print
            print "upgrading chained package: %s" % self.next_p.name
            retval = self.next_p.upgrade()
            self.next_p = self.next_p.next_p
...
class srp(package):
    ...
    """
    def __init__(self, filename, notes_name=sr.NOTES2):
        package.__init__(self, notes_name=notes_name)
        ...
        if "SRP_CHAIN" in self.srp_flags:
            utils.vprint("initializing self.next_p")
            self.next_p = srp(filename, notes_name=self.chain)
    ...
    def build_brp(self, filename, destination):
        ...
        # grab NOTES
        try:
            os.link(sr.RUCKUS + "/package/" + self.notes_name, sr.NOTES2)
        except Exception, e:
            print "[ failed ]"
            print "error copying NOTES file: %s" % e
            return 0
        ...
        if "SRP_PREPOSTLIB" in self.srp_flags:
            brp_contents.append(self.prepost_filename)
        if "SRP_OWNEROVERRIDE" in self.srp_flags:
        ...
        while self.next_p and retval:
            print
            print "building chained package: %s" % self.next_p.name
            retval = self.next_p.build_brp(filename, destination)
            self.next_p = self.next_p.next_p
    ...
    def _read_files(self):
        ...
        for i in self.srp_flags[:]:
            ...
            if x not in sr.supported_flags:
                ...
                #    utils.vprint("initializing self.chain")
                #    self.chain = y
                # ^^ SRP_CHAIN shouldn't get into a BRP ^^
                elif x == "SRP_PREPOSTLIB":
                    utils.vprint("initializing self.prepost_filename")
                    self.prepost_filename = y
