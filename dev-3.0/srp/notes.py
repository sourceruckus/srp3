...
class v3(utils.base_obj):
    def __init__(self, package_p, filename=config.NOTES):
        ...
        # first, extract the requested NOTES file from the package
        try:
            file_p = package_p.extractfile(filename)
        except Exception, e:
            err = "Failed to extract NOTES file '%s': %s" % (filename, e)
            raise Exception(err)

        # now, try parsing it
        try:
            c.readfp(file_p)
            self.__notes_version = c.get("header", "version")
            self.__name = c.get("info", "name")
            self.__version = c.get("info", "version")
            self.__revision = c.get("info", "revision")
            self.__sourcefilename = c.get("info", "sourcefilename")
            self.__description = c.get("info", "description")
            self.__flags = c.get("options", "flags").split()
            self.__parse_flags(package_p)
            self.__script = c.get("options", "script")
            ...
            err = "Failed to parse NOTES file '%s': %s" % (filename, e)
            raise Exception(err)

        # double check the notes_version
        if self.__notes_version != "3":
    ...
    def __parse_flags(self, package_p):
        utils.vprint("flags pre-parsing: %s" % (self.__flags))
        ...
        for i in self.__flags[:]:
            if "=" in i:
                ...
                x = flagarg[0]
                y = flagarg[1]
                self.__flags.remove(i)
                self.__flags.append(x)
            else:
                x = i
                y = ""
            if x not in config.SUPPORTED_FLAGS:
                ...
                self.__flags.remove(x)
            elif y != "":
                # do something with the flag arg
                if x == "SRP_PREPOSTLIB":
                    utils.vprint("initializing self.__prepostlib")
                    self.__prepostlib = y
                    # instantiate prepostlib_p now
                    self.__prepostlib_p = prepostlib.v3(package_p, y)

                elif x == "SRP_OWNEROVERRIDE":
                    utils.vprint("initializing self.__owneroverride")
                    self.__owneroverride = y
                    ...
                    utils.vprint("initializing self.__ldpath")
                    self.__ldpath = y.split(',')

                elif x == "SRP_CHAIN":
                    utils.vprint("initializing self.__chain")
                    self.__chain = y
                    
        self.__add_default_flags()
        self.__override_default_flags()
        
        utils.vprint("flags post-parsing: %s" % (self.__flags))


    def __add_default_flags(self):
        # add default_flags, if necesary
        for x in config.DEFAULT_FLAGS:
            if x not in self.__flags:
                # special case for CHECKSUM variants
                if x == "SRP_CHECKSUM":
                    if "SRP_MD5SUM" in self.__flags:
                        config.CHECKSUM = "md5"
                    elif "SRP_SHA1SUM" in self.__flags:
    ...
    def __override_default_flags(self):
        ...
        if "SRP_NO_UPGRADE" in self.__flags:
            self.__flags.remove("SRP_UPGRADABLE")
            self.__flags.remove("SRP_NO_UPGRADE")

        if "SRP_NO_CHECKSUM" in self.__flags:
            self.__flags.remove("SRP_CHECKSUM")
            self.__flags.remove("SRP_NO_CHECKSUM")

        if "SRP_NO_PERMS" in self.__flags:
            self.__flags.remove("SRP_PERMS")
            self.__flags.remove("SRP_NO_PERMS")

        if "SRP_NO_LINKTARGET" in self.__flags:
            self.__flags.remove("SRP_LINKTARGET")
            self.__flags.remove("SRP_NO_LINKTARGET")

        if "SRP_NO_INSTALLINFO" in self.__flags:
            self.__flags.remove("SRP_INSTALLINFO")
            self.__flags.remove("SRP_NO_INSTALLINFO")

        if "SRP_NO_LDCONFIG" in self.__flags:
            self.__flags.remove("SRP_LDCONFIG")
            self.__flags.remove("SRP_NO_LDCONFIG")
